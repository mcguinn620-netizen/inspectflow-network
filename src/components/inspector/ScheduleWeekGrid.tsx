  ...
  return (
    <div className="grid grid-cols-7 gap-1.5">
      {days.map((d) => {
        ...
        const dayConflictCount = dayJobs.filter((j) => conflicts.has(j.id)).length;
        return (
          <Card
            key={d.toISOString()}
            className={cn(
          ...
          >
            ...
            <div className="space-y-1">
              {dayJobs.slice(0, 4).map((j) => {
                const reasons = conflicts.get(j.id);
                const hasConflict = !!reasons?.length;
                return (
                  <button
                    key={j.id}
                    type="button"
                    ...
                    className={cn(
                      ...
                      hasConflict && "border-l-2 border-l-destructive",
                    )}
                    title={hasConflict ? summarizeConflicts(reasons!) : j.title}
                  >
                    {hasConflict && (
                      <AlertTriangle className="h-2.5 w-2.5 inline-block mr-1 text-destructive" />
                    )}
